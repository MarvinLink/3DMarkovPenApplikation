...
namespace VRSketchingGeometry.Splines
{
    ...
    public class KochanekBartelsSpline : Spline
    {
        ...
        public KochanekBartelsSpline(int smoothness = 4 ,int steps = 20) {
        ...
        private KochanekBartelsControlPoint LastInput = null;
        /// <summary>
        ...
        public SplineModificationInfo AddControlPoint(KochanekBartelsControlPoint controlPoint) {
            ...
            {
                return InsertControlPoint(0, controlPoint);
              
            }

            if (LastInput == null)
            {
                LastInput = controlPoint;
                return new SplineModificationInfo(0, 0, 0);
            }


            KochanekBartelsControlPoint interpolatedPoint =
                _smoothness == 1 
                    ? controlPoint
                    : InterpolateSegment(ControlPoints.Count == 1? ControlPoints[^1] : ControlPoints[^2], ControlPoints[^1], LastInput, controlPoint, _smoothness)[1];
            LastInput = controlPoint;

            return InsertControlPoint(ControlPoints.Count, interpolatedPoint);
